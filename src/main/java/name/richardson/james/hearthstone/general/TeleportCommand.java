...
public class TeleportCommand extends PluginCommand {
  ...
  public void parseArguments(List<String> arguments, CommandSender sender) throws CommandArgumentException {
    ...
    if (sender instanceof ConsoleCommandSender) {
      throw new CommandArgumentException(this.plugin.getMessage("command-is-player-only"), 
                                         this.plugin.getMessage("teleportcomnmand-teleport-self-only"));
    }
    
    final UUID defaultWorldId = ((Player) sender).getWorld().getUID();
    
    // if the arguments are empty do nothing and assume the user is teleporting themselves
    if (!arguments.isEmpty()) {
      ...
      // check to see if the world is loaded and if it is not default with players current world.
      } else {
